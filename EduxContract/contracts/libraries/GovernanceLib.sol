...
library GovernanceLib {
    ...
    function setTreasuryFeeBPS(uint16 newTreasuryFee) internal {
        ...
        emit Events.TreasuryFeeSetBPS(prevTreasuryFee, newTreasuryFee, block.timestamp);
    }

    function setTreasuryFee(uint256 newTreasuryFee, address currency) internal {
        Types.TreasuryData storage _treasuryData = StorageLib.getTreasuryData();

        uint256 prevTreasuryFee = _treasuryData.treasuryFee[currency];
        _treasuryData.treasuryFee[currency] = newTreasuryFee;

        emit Events.TreasuryFeeSet(prevTreasuryFee, newTreasuryFee, currency, block.timestamp);
    }
